...
from pyfvvdp.fvvdp_lpyr_dec import fvvdp_lpyr_dec, fvvdp_contrast_pyr, fvvdp_spyr_dec
...
class fvvdp:
    ...
    def predict_video_source(self, vid_source, fixation_point=None):
        ...
        if self.lpyr is None or self.lpyr.W!=width or self.lpyr.H!=height:
            ...
            elif self.local_adapt=="spyr":
                self.lpyr = fvvdp_spyr_dec(width, height, self.pix_per_deg, self.device)
            else:
                self.lpyr = fvvdp_lpyr_dec(width, height, self.pix_per_deg, self.device)

            if self.do_heatmap:
                if self.local_adapt=="spyr":
                    self.heatmap_pyr = fvvdp_spyr_dec(width, height, self.pix_per_deg, self.device)
                else:
        ...
        for ff in range(N_frames):
            ...
            for cc in range(temp_ch):
                for bb in range(self.lpyr.get_band_count()):  # I removed the -1
                    ...
                    if Q_per_ch is None:
                        Q_per_ch = torch.zeros((self.lpyr.height+1, 2, N_frames), device=self.device)
