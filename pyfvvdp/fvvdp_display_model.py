...
class fvvdp_display_photometry:
    ...
    def load(cls, display_name):
        ...
        obj = fvvdp_display_photo_eotf(Y_peak, contrast=contrast, gamma=gamma, EOTF=EOTF, E_ambient=E_ambient,
                                       k_refl=k_refl, name=display_name)
...
class fvvdp_display_geometry:
    ...
    def __init__(self, resolution, distance_m=None, distance_display_heights=None, fov_horizontal=None,
                 fov_vertical=None, fov_diagonal=None, diagonal_size_inches=None) -> None:
        ...
        if not distance_m is None:
            ...
            if not hasattr(self, "display_size_m"):
                raise RuntimeError(
                    'You need to specify display diagonal size ''diagonal_size_inches'' to specify viewing distance as ''distance_display_heights'' ')
        ...
        if ((not fov_horizontal is None) + (not fov_vertical is None) + (not fov_diagonal is None)) > 1:
            raise RuntimeError(
                'You can pass only one of ''fov_horizontal'', ''fov_vertical'', ''fov_diagonal''. The other dimensions are inferred from the resolution assuming that the pixels are square.')
        ...
        if not fov_horizontal is None:
            ...
            distance_px = math.sqrt(self.resolution[0] ** 2 + self.resolution[1] ** 2) / (
                        2.0 * math.tan(math.radians(fov_diagonal * 0.5)))
    ...
    def pix2eccentricity(self, resolution_pix, x_pix, y_pix, gaze_pix):
        ...
        if not self.fixed_ppd is None:
            ...
            ecc = torch.sqrt((torch.rad2deg(torch.atan(x_m / self.distance_m)) - gaze_deg[0]) ** 2 + (
                        torch.rad2deg(torch.atan(y_m / self.distance_m)) - gaze_deg[1]) ** 2)
    ...
    def print(self):
        ...
        if hasattr(self, "fixed_ppd"):
            ...
            logging.info('  Display size: {w:.1f} x {h:.1f} cm'.format(w=self.display_size_m[0] * 100,
                                                                       h=self.display_size_m[1] * 100))
            logging.info(
                '  Display size: {w:.2f} x {h:.2f} deg'.format(w=self.display_size_deg[0], h=self.display_size_deg[1]))
    ...
    def load(cls, display_name):
        ...
        for mk in models:
            if mk == display_name:
                ...
                if "fov_diagonal" in model:
                    fov_diagonal = model["fov_diagonal"]
                else:
                    fov_diagonal = None

                if "viewing_distance_meters" in model:
                    distance_m = model["viewing_distance_meters"]
                elif "viewing_distance_inches" in model:
                    distance_m = model["viewing_distance_inches"] * inches_to_meters
                else:
                    distance_m = None

                if "diagonal_size_meters" in model:
                    diag_size_inch = model["diagonal_size_meters"] / inches_to_meters
                elif "diagonal_size_inches" in model:
                    diag_size_inch = model["diagonal_size_inches"]
                else:
                    diag_size_inch = None

                obj = fvvdp_display_geometry((W, H), distance_m=distance_m, fov_diagonal=fov_diagonal,
                                             diagonal_size_inches=diag_size_inch)
                return obj
